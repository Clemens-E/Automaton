...
module.exports.run = async (client, message) => {
    ...
    const msg = await message.channel.send(`${client.infos.updating} executing pull command...`);
    exec('git pull origin master', async (err, stdout) => {
        if (err) throw err;
        if (stdout === 'Already up-to-date.\n') return msg.edit(stdout);
        await msg.edit(`${client.infos.updating}\`\`\`fix\n${stdout}\`\`\`${client.infos.loading}Now restarting`);
